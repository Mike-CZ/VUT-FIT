...
tHTItem* htSearch ( tHTable* ptrht, tKey key ) {
	...
	int index = hashCode(key);					// index do tabulky symbolu

	if ((*ptrht)[index] == NULL){				// pokud takova polozka neexistuje vratim null
		return NULL;
	}

	if (!strcmp((*ptrht)[index]->key, key)){	// pokud hledana polozka se nachazi na tomto indexu, vratim ji
		return (*ptrht)[index];
	}
	...
	while (tmp->ptrnext != NULL){				// hledam prvek podle klice
...
void htInsert ( tHTable* ptrht, tKey key, tData data ) {
	...
	tHTItem *tmp;	// pomocna promenna
	if ((tmp = htSearch(ptrht, key)) != NULL){			// pokud prvek jiz existuje, aktualizuji data
		tmp->data = data;
		return;
	}
	...
	if ((tmp = malloc(sizeof(struct tHTItem))) == NULL){
		return;
	}
	if ((tmp->key = malloc(sizeof(char)* (strlen(key) + 1))) == NULL){
		return;
	}

	tmp->data = data;				// vlozim data
	strcpy(tmp->key, key);

	tmp->ptrnext = (*ptrht)[index];		// vlozim do tabulky
	(*ptrht)[index] = tmp;
}
...
void htDelete ( tHTable* ptrht, tKey key ) {
	...
	if ((*ptrht)[index] == NULL){  // pokud prvek neexistuje nic nedelam
		return;
	}

	tHTItem *tmp;
	tmp = (*ptrht)[index];
	if (!strcmp((*ptrht)[index]->key, key)){	// pokud je hledany prvek prvni v seznamu tak jej uvolnim
	...
	while (tmp->ptrnext != NULL){
		tmp2 = tmp;							//potrebuji dalsi pomocnou promennou
		tmp = tmp->ptrnext;

		if (!strcmp(tmp->key, key)){
			tmp2->ptrnext = tmp->ptrnext;	// kdyt najdu shodu, smazu prvek
			free(tmp->key);
			free(tmp);
			return;
		}
...
void htClearAll ( tHTable* ptrht ) {
	...
	for (int x = 0; x < HTSIZE; x++){	// projedu cele pole

		tmp = (*ptrht)[x];

		while (tmp != NULL){			// projedu celej seznam na danem indexu pole
			tmp2 = tmp->ptrnext;
			free(tmp->key);
			free(tmp);
			tmp = tmp2;
