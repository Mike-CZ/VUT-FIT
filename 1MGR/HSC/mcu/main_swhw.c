...
void print_results(long frame, long threshold, unsigned long *hist, int n) {
	int i;
...
int main(void)
{
   ...
   int prev_frame = 0;
   int x = 0;
   unsigned long  histogram[PIXELS] = { 0, 0, 0, 0, 0, 0, 0, 0 };
   ...
   while (frame < FRAMES) {
		   ...
		   if (frame != prev_frame && frame % 10 == 0) {

			   // zkopiruj histogram a vynuluj histogram na sdilene sbernici
			   for (x = 0; x < PIXELS; x++) {
				   histogram[x] = fpga_read(FPGA_HISTOGRAM + x);
				   fpga_write(FPGA_HISTOGRAM + x, 0);
			   }
