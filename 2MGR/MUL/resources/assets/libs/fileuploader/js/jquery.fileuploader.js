...
(function($) {
	...
	$.fn.fileuploader.defaults = {
                ...
                template: function(data) { return '<div class="fileuploader-popup">' +
                    '<div class="fileuploader-popup-preview">' +
                        ...
                        '<div class="tools">' +
                            ...
                            '</ul>' +
                            '<div class="buttons">' +
                                '<a class="if-btn if-btn--primary-color if-btn--sw if-btn--lh" data-action="cancel">${captions.cancel}</a>' +
                                '<a class="if-btn if-btn--secondary-color if-btn--sw if-btn--lh" data-action="save">${captions.confirm}</a>' +
                            '</div>' +
                        '</div>' +
                    '</div>' +
                '</div>'; },
        ...
        captions: {
            ...
            feedback2: function(options) { return options.length + ' ' + (options.length > 1 ? ' soubory byly vybrány.' : ' soubor byl vybrán.'); },
			confirm: 'Potvrdit',
            cancel: 'Zrušit',
			name: 'Jméno',
			type: 'Typ',
            ...
            download: 'Stáhnout',
            remove: 'Odstranit',
            drop: 'Přetáhněte soubory k nahrání.',
            paste: '<div class="fileuploader-pending-loader"><div class="left-half" style="animation-duration: ${ms}s"></div><div class="spinner" style="animation-duration: ${ms}s"></div><div class="right-half" style="animation-duration: ${ms}s"></div></div> Kliknutím nahrávání souboru zrušíte.',
            removeConfirmation: 'Opravdu chcete tento soubor odstranit?',
            errors: {
                filesLimit: 'Je možné nahrát maximálně ${limit} souborů.',
                filesType: 'Jsou povoleny následující typy souborů: ${extensions}.',
                fileSize: 'Soubor ${name} je příliš velký! Prosím, nahrajte soubor o maximální velikosti ${fileMaxSize}MB.',
                filesSizeAll: 'Vybrané soubory jsou příliš velké! Prosím, nahrajte soubory do velikosti ${maxSize} MB.',
                fileName: 'Soubor s názvem ${name} byl již nahrán.',
                folderUpload: 'Nemůžete nahrávat soubory.'
            }
        }
	}
})(jQuery);