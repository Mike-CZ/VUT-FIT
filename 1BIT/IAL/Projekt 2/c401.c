...
int BSTSearch(tBSTNodePtr RootPtr, char K, int *Content)	{
	...
	**/
	...
	if (RootPtr == NULL){
		return FALSE;			// pokud je uzel prazdny, vracim false
	}
	
	if (RootPtr->Key != K){
		if (RootPtr->Key > K){
			return BSTSearch(RootPtr->LPtr, K, Content);	// pokud je hledany klic mensi, zanorim se doleva
		}
		else{
			return BSTSearch(RootPtr->RPtr, K, Content);	// jinak se zanorim doprava
		}
	}
	else{
		*Content = RootPtr->BSTNodeCont;			// pokud jsem nasel uzel, vratim obsah
		return TRUE;
	}
...
**/
	...
	if (*RootPtr == NULL){
		tBSTNodePtr uzel;		// pokud vkladam do prazdneho stromu, vlozim novy uzel

		if ((uzel = malloc(sizeof(struct tBSTNode))) == NULL){
			return;
		}
	...
	else if ((*RootPtr)->Key > K){					// Pokud je klic vetsi, zanorim se doleva
		BSTInsert(&(*RootPtr)->LPtr, K, Content);
	}
	else if ((*RootPtr)->Key < K){
		BSTInsert(&(*RootPtr)->RPtr, K, Content);	// Pokud je klic mensi zanorim se doprava
	}
	else{
		(*RootPtr)->BSTNodeCont = Content;				// Jinak vlozim hodnotu
	}
}
...
**/
	...
	if ((*RootPtr)->RPtr != NULL){
		ReplaceByRightmost(PtrReplaced, &(*RootPtr)->RPtr);		//hledam nejpravejsi prvek
	}
	else{
		PtrReplaced->Key = (*RootPtr)->Key;
		PtrReplaced->BSTNodeCont = (*RootPtr)->BSTNodeCont;
		temp = *RootPtr;										//nahrazeni prvku
...
**/
	...
	if (*RootPtr != NULL){
		if ((*RootPtr)->Key > K){
			BSTDelete(&(*RootPtr)->LPtr, K);	// hledam klic
		}
		else if ((*RootPtr)->Key < K){
			...
			tmp = *RootPtr;
			if ((*RootPtr)->RPtr == NULL){
				*RootPtr = (*RootPtr)->LPtr;		//prehodim ukazatel a uvolnim
				free(tmp);
			}
			else if ((*RootPtr)->LPtr == NULL){		// to stejne pro druhou vetev
...
/*    ----------
** Z ru¹í celý binární vyhledávací strom a korektnì uvolní pamì».
**  
...
**/
	...
	if (*RootPtr != NULL){
		BSTDispose(&(*RootPtr)->RPtr);		// zanorim se do prave vetve
		BSTDispose(&(*RootPtr)->LPtr);		// zanorim se do leve vetve
		free(*RootPtr);						// uvolnim prvek
		*RootPtr = NULL;					// priradim ukazatel do NULL
	}
}
